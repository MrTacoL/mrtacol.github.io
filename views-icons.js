(() => {
  try {
    ...
    style.textContent = `
      ...
      .device-icon.safe-game-icon {
        background-image: none !important;
        color: var(--blue) !important;
      }
      .device-icon.safe-game-icon svg {
    ...
    function compact(n) {
      return new Intl.NumberFormat('en', { notation: 'compact', maximumFractionDigits: 1 }).format(Number(n || 1));
    }

    if (viewCount) {
      const key = 'mrtacosi:profile:safeViews';
      const seenKey = 'mrtacosi:profile:safeSeen';
      ...
      viewCount.textContent = compact(views);
    }

    function iconForText() {
    ...
    function updateIconOnce() {
      if (!activityIcon) return;
      const [key, svg] = iconForText();
      if (activityIcon.dataset.safeIcon === key) return;
      activityIcon.dataset.safeIcon = key;
      activityIcon.classList.remove('has-art');
      activityIcon.classList.add('safe-game-icon');
      activityIcon.style.backgroundImage = 'none';
      activityIcon.innerHTML = svg;
    }

    setTimeout(updateIconOnce, 600);
    setTimeout(updateIconOnce, 1600);
    setTimeout(updateIconOnce, 3500);
  } catch (error) {
    console.warn('safe site addon stopped:', error);
  }
})();
