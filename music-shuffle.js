(() => {
  ...
  css.textContent = `
    ...
    .music-drag-handle{display:none;align-items:center;justify-content:center;height:1.5rem;margin:-.15rem 0 .55rem;border-radius:999px;background:rgba(255,255,255,.09);color:rgba(255,255,255,.72);font-size:.62rem;font-weight:950;letter-spacing:.08em;text-transform:uppercase;cursor:grab}.music-dragging .music-drag-handle{cursor:grabbing;background:rgba(101,239,255,.16)}@media(min-width:651px){.music-drag-handle{display:flex}}
  ...
  panel.innerHTML = `
    <div class="music-drag-handle">↕ Drag player</div>
  ...
  const dragHandle = panel.querySelector('.music-drag-handle');
  ...
  function setupDrag(){
    ...
    if(!soundPanel || !dragHandle) return;
    let startX=0,startY=0,startLeft=0,startTop=0,drag=false;
    dragHandle.addEventListener('pointerdown',(e)=>{
      const rect=soundPanel.getBoundingClientRect();
      drag=true;startX=e.clientX;startY=e.clientY;startLeft=rect.left;startTop=rect.top;
      soundPanel.classList.add('music-dragging');
      ...
      dragHandle.setPointerCapture(e.pointerId);e.preventDefault();
    });
    dragHandle.addEventListener('pointermove',(e)=>{
      if(!drag) return;
      const width=soundPanel.offsetWidth,height=soundPanel.offsetHeight;
      const left=Math.max(8,Math.min(window.innerWidth-width-8,startLeft+e.clientX-startX));
      const top=Math.max(8,Math.min(window.innerHeight-height-8,startTop+e.clientY-startY));
      soundPanel.style.left=left+'px';soundPanel.style.top=top+'px';
    });
    function stop(){if(!drag)return;drag=false;soundPanel.classList.remove('music-dragging');localStorage.setItem('musicPlayerPosition',JSON.stringify({left:parseFloat(soundPanel.style.left)||16,top:parseFloat(soundPanel.style.top)||16}));}
    dragHandle.addEventListener('pointerup',stop);dragHandle.addEventListener('pointercancel',stop);
    const saved=JSON.parse(localStorage.getItem('musicPlayerPosition')||'null');
    if(saved&&window.matchMedia('(min-width:651px)').matches){soundPanel.style.left=saved.left+'px';soundPanel.style.top=saved.top+'px';soundPanel.style.bottom='auto';soundPanel.style.transform='none';}
  }
  setupDrag();
